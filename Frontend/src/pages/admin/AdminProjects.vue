  ...
  <div>
    ...
    <template v-else>
      ...
      <v-data-table
        ...
        class="admin-table admin-table--featured mb-6"
        hover
        hide-default-footer
        :items-per-page="-1">
        <template #item="{ item }">
          <tr
            ...
            <td v-if="isDraggable" class="drag-cell" draggable="true"
              @dragstart="onDragStart($event, item)" @dragend="onDragEnd">
              <v-icon size="18" class="drag-handle">mdi-drag-vertical</v-icon>
            </td>
            <td>{{ item.title }}</td>
            <td><span class="text-body-2">{{ works.find(w => w.workExperienceId === item.workExperienceId)?.employer ?? '—' }}</span></td>
            <td>{{ item.role }}</td>
            <td>{{ item.displayOrder }}</td>
            <td><v-icon color="yellow" size="18">mdi-star</v-icon></td>
            <td><v-chip :color="item.isActive ? 'success' : 'error'" size="x-small" variant="tonal">{{ item.isActive ? 'Active' : 'Inactive' }}</v-chip></td>
            <td><v-icon :color="item.hasEmbedding ? 'success' : 'warning'" size="18">{{ item.hasEmbedding ? 'mdi-brain' : 'mdi-brain-off' }}</v-icon></td>
            <td>
              <div class="chip-wrap">
            ...
            <td>
              <div class="row-actions">
                <v-btn icon size="x-small" variant="text" @click="openDialog(item)" title="Edit"><v-icon>mdi-pencil</v-icon></v-btn>
                <v-btn icon size="x-small" variant="text" color="secondary" :loading="embeddingItemLoading === item.projectId" @click="regenerateEmbedding(item.projectId)"><v-icon>mdi-refresh</v-icon></v-btn>
                <v-btn icon size="x-small" variant="text" color="error" @click="toggleActive(item)" :title="item.isActive ? 'Deactivate' : 'Restore'"><v-icon>{{ item.isActive ? 'mdi-eye-off' : 'mdi-eye' }}</v-icon></v-btn>
              </div>
            </td>
          </tr>
        </template>
      ...
      <div class="section-header section-header--standard">
        <div class="section-header__left">
          <v-icon size="13" class="mr-1" style="color: rgba(var(--v-theme-on-surface), 0.35)">mdi-view-grid-outline</v-icon>
          ...
          <tr
            ...
            <td>
              <div class="row-actions">
                <v-btn icon size="x-small" variant="text" @click="openDialog(item)" title="Edit"><v-icon>mdi-pencil</v-icon></v-btn>
                <v-btn icon size="x-small" variant="text" color="secondary" :loading="embeddingItemLoading === item.projectId" @click="regenerateEmbedding(item.projectId)"><v-icon>mdi-refresh</v-icon></v-btn>
                <v-btn icon size="x-small" variant="text" :color="item.isActive ? 'error' : 'success'" @click="toggleActive(item)" :title="item.isActive ? 'Deactivate' : 'Restore'"><v-icon>{{ item.isActive ? 'mdi-eye-off' : 'mdi-eye' }}</v-icon></v-btn>
              </div>
            </td>
          </tr>
        </template>
...
<style scoped>
.tab-toolbar { display: flex; align-items: center; gap: 0.75rem; margin-bottom: 0.75rem; flex-wrap: wrap; }
...
/* ── Reorder status ── */
.reorder-saving, .reorder-saved {
  display: flex;
  align-items: center;
  font-size: 0.75rem;
  font-family: 'Raleway', sans-serif;
}
.reorder-saving { color: rgba(var(--v-theme-secondary), 0.7); }
.reorder-saved  { color: rgb(var(--v-theme-success)); }
...
.section-label--featured { color: rgba(255, 196, 0, 0.8); }
.section-label--standard { color: rgba(var(--v-theme-on-surface), 0.35); }
